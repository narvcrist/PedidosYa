    ...
    if(!empty($_POST)){
        ...
        if($_POST['action'] == 'addProductoDetalle'){
            if(empty($_POST['producto']) || empty($_POST['cantidad'])){
                ...
                if($result >0){
                    ...
                    $detalleTotales = ' <tr>
                                            <td id="iz" colspan="5">SUBTOTAL $.</td>
        ...
        if($_POST['action'] == 'infoFactura'){
            if(!empty($_POST['nofactura'])){
                ...
                $consulta = mysqli_query($conection,"SELECT * FROM factura WHERE fac_id = '$nofactura' AND estado = 1");
                mysqli_close($conection);
        ...
        if($_POST['action'] == 'anularFactura'){
            if(!empty($_POST['noFactura'])){
                ...
                $consulta = mysqli_query($conection,"CALL anular_factura($noFactura)");
                mysqli_close($conection);
                $result = mysqli_num_rows($consulta);
                if($result > 0){
                    $data = mysqli_fetch_assoc($consulta);
